...
const IOS = Platform.PlatformOSType === 'ios';
const ANDROID = Platform.PlatformOSType === 'android';
...
export default class Home extends Component {
  state = {
    ...
    tipoUnidades: [
      ...
      {
        cod: '02',
        descricao: 'CENTRO DE SAUDE/UNIDADE BASICA',
        icone: 'clinic-medical',
      },
  ...
  handleclick = async () => {
    ...
    const listaPlace = JSON.stringify(
      est['soap:Envelope']['S:Body'][0][
        'est:responseLocalizarEstabelecimentoSaude'
      ][0]['res:ResultadosLocalizacaoEstabelecimentoSaude'][0][
        'res1:ResultadoLocalizacaoEstabelecimentoSaude'
      ],
    );
      ...
      this.setState({
        estabelecimentos: [
          ...this.state.estabelecimentos,
          {
            ...
            logradouro:
              place[PLACE][0]['end:Endereco'][0]['ns11:nomeLogradouro'][0]._,
            numero: place[PLACE][0]['end:Endereco'][0]['ns11:numero'][0]._,
            bairro:
              place[PLACE][0]['end:Endereco'][0]['ns11:Bairro'][0][
                'ns13:descricaoBairro'
              ][0],
            cep:
              place[PLACE][0]['end:Endereco'][0]['ns11:CEP'][0][
                'ns14:numeroCEP'
              ][0],
            municipio:
              place[PLACE][0]['end:Endereco'][0]['ns11:Municipio'][0][
                'ns15:nomeMunicipio'
              ][0],
            uf:
              place[PLACE][0]['end:Endereco'][0]['ns11:Municipio'][0][
            ...
            longitude:
              place[PLACE][0]['dad:Localizacao'][0]['ns30:longitude'][0]._,
            latitude:
              place[PLACE][0]['dad:Localizacao'][0]['ns30:latitude'][0]._,
  ...
  handleSelectedItem = item => {
    ...
    const _TiposEstabelecimentos = this.state.tipoUnidades.filter(
      id => id.cod !== item.cod,
    );
    ...
    return (
      <Container>
        ...
        {estabelecimentos.length > 0 && (
          ...
          <LinearColor>
            <View
              style={{flexDirection: 'row', paddingLeft: 20, paddingTop: 10}}>
              <Icon
                name="hospital-alt"
                size={30}
                color="#000"
              ...
              <View style={{marginRight: 30}}>
                <Text style={{fontSize: 18, fontWeight: 'bold', maxWidth: 290}}>
                  {estabelecimentos[this.state.calloutSelect].nome}
                </Text>
                <View style={{flexDirection: 'row'}}>
                  <View>
                    <Text>
                      {estabelecimentos[this.state.calloutSelect].logradouro}
                    </Text>
                    <Text>
                      {estabelecimentos[this.state.calloutSelect].bairro}
                    </Text>
                    <Text>
                      {estabelecimentos[this.state.calloutSelect].numero}
                    </Text>
                    <Text>
                    ...
                    <Text>
                      {estabelecimentos[this.state.calloutSelect].cep}
                    </Text>
                    <View
                      style={{
                        flexDirection: 'row',
                  ...
                  <View style={{justifyContent: 'center'}}>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        Linking.openURL(
                          `geo:${this.state.region}?q=${
                            this.state.estabelecimentos[
                              this.state.calloutSelect
                            ].latitude
                          },${
                            this.state.estabelecimentos[
                              this.state.calloutSelect
        ...
        {/** Botão Pesquisa */}
        {unidadeSelecionada.cod && !exibirListadeResultados && (
          <BtnLinear>
          ...
          <ViewFooter>
            <ScrollView
              ...
              onMomentumScrollEnd={e => {
                ...
                const {latitude, longitude, mark} = estabelecimentos[
                  Math.round(place)
                ];
                this.mapView.animateCamera(
                  {
