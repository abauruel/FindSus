...
class SoapRequest {
  constructor(props) {
    if (props.security) {
      if (props.security.username && props.security.password) this.security = props.security;
  ...
  createRequest(request) {
    this.xmlDoc = new DOMParser().parseFromString(
      '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"></soap:Envelope>',
    );
    this.rootElement = this.xmlDoc.documentElement;

    if (this.targetNamespace) {
      this.targetNamespace.map((namespace) => {
        const { xmlns, url } = namespace;
        this.rootElement.setAttribute(xmlns, url);
      });
    }
  ...
  eachRecursive(obj, parentElement) {
    ...
    const currentElement = parentElement;

    for (const k in obj) {
      if (!obj.hasOwnProperty(k)) continue; // skip this property

      if (typeof obj[k] === 'object' && obj[k] !== null) {
        if (Object.keys(obj[k]).find((x) => x == 'attributes')) {
          for (const attr in obj[k].attributes) {
  ...
  appendChild(parentElement, name, text) {
    const childElement = this.xmlDoc.createElement(name);
    if (typeof text !== 'undefined') {
      const textNode = this.xmlDoc.createTextNode(text);
  ...
  generateHeader() {
    ...
    if (this.security) {
      ...
      const expiresElement = this.appendChild(
        timestampElement,
        'wsu:Expires',
        moment(date)
          .add(10, 'm')
      ...
      const usernameElement = this.appendChild(
        usernameTokenElement,
        'wsse:Username',
        this.security.username,
      );
      const passwordElement = this.appendChild(
        usernameTokenElement,
        'wsse:Password',
  ...
  async sendRequest() {
    if (!this.xmlRequest) { throw new Error('Request empty, please call createRequest before sendRequest'); }
    if (!this.requestURL) throw new Error('requestURL empty!');
